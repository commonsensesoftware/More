namespace More.Composition
{
    ...
    {
        readonly Func<string, Type, object> locate;
        ...
        static bool TryGetKey( CompositionContract contract, out string key, out Type targetType )
        {
            Contract.Requires( contract != null );

            targetType = null;

            if ( !contract.TryUnwrapMetadataConstraint( nameof( SettingAttribute.Key ), out key, out var constraint ) )
            {
                return false;
            }
        ...
        private object GetValue( CompositionContract contract, string key )
        {
            ...
            if ( contract.TryUnwrapMetadataConstraint( nameof( SettingAttribute.DefaultValue ), out value, out var constraint ) && value != NullValue )
            {
                return value;
            }
        ...
        static CompositeActivator CreateActivator( Func<object> resolve, Type targetType ) => ( c, o ) => Convert( resolve(), targetType, CurrentCulture );
        ...
        {
            if ( !TryGetKey( contract, out var key, out var targetType ) )
            {
