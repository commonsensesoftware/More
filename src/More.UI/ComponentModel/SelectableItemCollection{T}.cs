namespace More.ComponentModel
{
    ...
    public class SelectableItemCollection<T> : ReadOnlyObservableCollection<SelectableItem<T>>
    {
        ...
        readonly SelectAllItem<T> selectAllItem;
        readonly SelectedValuesCollection<T> selectedValues;
        readonly ObservableCollection<SelectableItem<T>> selectedItems;
        readonly ReadOnlyObservableCollection<SelectableItem<T>> readOnlySelectedItems;
        readonly Func<T, IEqualityComparer<T>, SelectableItem<T>> factory;

        /// <summary>
        /// Initializes a new instance of the <see cref="SelectableItemCollection{T}"/> class.
        /// </summary>
        protected SelectableItemCollection()
            : this( ( v, c ) => new SelectableItem<T>( v, c ), new T[0], default( object ), EqualityComparer<T>.Default ) { }
        ...
        protected SelectableItemCollection( IEqualityComparer<T> comparer )
            : this( ( v, c ) => new SelectableItem<T>( v, c ), Enumerable.Empty<T>(), default( object ), comparer ) { }
        ...
        protected SelectableItemCollection( Func<T, IEqualityComparer<T>, SelectableItem<T>> factory, IEqualityComparer<T> comparer )
            : this( factory, Enumerable.Empty<T>(), default( object ), comparer ) { }
        ...
        public SelectableItemCollection( IEnumerable<T> sequence )
            : this( ( v, c ) => new SelectableItem<T>( v, c ), sequence, default( object ), EqualityComparer<T>.Default ) { }
        ...
        public SelectableItemCollection( IEnumerable<T> sequence, IEqualityComparer<T> comparer )
            : this( ( v, c ) => new SelectableItem<T>( v, c ), sequence, default( object ), comparer ) { }
        ...
        public SelectableItemCollection( IEnumerable<T> sequence, T selectAllValue )
            : this( ( v, c ) => new SelectableItem<T>( v, c ), sequence, selectAllValue, EqualityComparer<T>.Default ) { }
        ...
        public SelectableItemCollection( IEnumerable<T> sequence, T selectAllValue, IEqualityComparer<T> comparer )
            : this( ( v, c ) => new SelectableItem<T>( v, c ), sequence, selectAllValue, comparer ) { }
        ...
        sealed class SelectedValuesMediator<TItem> : IDisposable
        {
            readonly ObservableCollection<TItem> source;
            readonly INotifyCollectionChanged target;
            readonly ICollection<TItem> targetCollection;
            bool disposed;

            ~SelectedValuesMediator() => Dispose( false );
        ...
        sealed class SelectAllItem<TValue> : SelectableItem<TValue>
        {
            readonly SelectableItemCollection<TValue> owner;
            bool suppressEvents;
            bool processing;

            internal SelectAllItem( SelectableItemCollection<TValue> owner, TValue value ) : base( false, value )
            {
            ...
            protected override void OnPropertyChanged( PropertyChangedEventArgs e )
            {
                ...
                if ( suppressEvents ||
                    !string.Equals( e.PropertyName, nameof( IsSelected ), StringComparison.Ordinal ) ||
                    IsSelected == null )
                {
                    return;
                }
                ...
                foreach ( var item in owner.Where( i => !ReferenceEquals( this, i ) ) )
                {
        ...
        sealed class ItemCollection<TItem> : ObservableCollection<SelectableItem<TItem>>
        {
            readonly IEqualityComparer<TItem> valueComparer;
            readonly IEqualityComparer<SelectableItem<TItem>> itemComparer;
            SelectableItemCollection<TItem> owner;
            ...
            internal void Initialize( SelectableItemCollection<TItem> source, IEnumerable<TItem> sequence )
            {
                ...
                if ( sequence is INotifyCollectionChanged notifyCollection )
                {
                    notifyCollection.CollectionChanged += OnSourceCollectionChanged;
                }
            }

            void AddNewItems( IEnumerable<TItem> newItems )
            {
                var count = Count;
            ...
            void RemoveOldItems( IEnumerable<TItem> oldItems )
            {
                var count = Count;
            ...
            void ReplaceItems( int startingIndex, IEnumerable<TItem> newValues )
            {
                var any = false;
            ...
            internal void OnItemChanged( object sender, PropertyChangedEventArgs e )
            {
                ...
                if ( item.IsSelected ?? false )
                {
                    ...
                    if ( !ReferenceEquals( owner.selectAllItem, item ) )
                    {
                        // add selected value
        ...
        sealed class SelectedValuesCollection<TValue> : ObservableCollection<TValue>
        {
            readonly IEqualityComparer<TValue> comparer;
            readonly SelectableItemCollection<TValue> owner;
            ...
            bool ItemExistsInSource( TValue value ) => FindAll( value ).Any();

            IEnumerable<SelectableItem<TValue>> FindAll( TValue value )
            {
