namespace More
{
    ...
    public class WeakDelegate<T> : WeakDelegate where T : class
    {
        ...
        public WeakDelegate( T strongDelegate ) : base( GetDelegate( strongDelegate ) ) { }

        static Delegate GetDelegate( T strongDelegate )
        {
            ...
            if ( @delegate == null )
            {
                throw new ArgumentException( SR.InvalidArgType.FormatDefault( typeof( Delegate ) ), nameof( strongDelegate ) );
            }
        ...
        public virtual T CreateTypedDelegate() => CreateDelegate() as T;
    }
}