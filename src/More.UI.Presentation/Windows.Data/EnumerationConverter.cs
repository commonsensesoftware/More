namespace More.Windows.Data
{
    ...
    public partial class EnumerationConverter : FormatConverter
    {
        ...
        public bool UseNumericFormat { get; set; }

        static object GetEnumFromString( Type type, string text )
        {
        ...
        static Enum GetParameterAsEnum( Type enumType, object parameter )
        {
            if ( parameter == null )
            {
                throw new ArgumentException( ExceptionMessage.EnumConverterParameterExpected, nameof( parameter ) );
            }
            ...
            catch ( ArgumentException )
            {
                ...
                throw new ArgumentException( ExceptionMessage.EnumConverterParameterInvalid.FormatDefault( enumType, parameter ), nameof( parameter ) );
            }
...
#endif
        {
            ...
            var numeric = IsNumericType( targetType, out var underlyingType, out var nullable );
            ...
            if ( !supported && numeric )
            {
                ...
                if ( !UseNumericFormat )
                {
                    throw new ArgumentException( ExceptionMessage.NumericFormatMustBeUsedToConvertEnumToNumber.FormatDefault( targetType ), nameof( targetType ) );
                }
            ...
            if ( boolean )
            {
                return Equals( actualValue, GetParameterAsEnum( actualValue.GetType(), parameter ) );
            }
...
#endif
        {
            ...
            var numeric = IsNumericType( sourceType, out var underlyingType, out var nullable );
