namespace System.Windows.Data
{
    ...
    /// adaptedBuilder.BuildUp( columns );
    /// 
    /// ]]></code></example>
    public partial class DataGridColumnBuilder<T> : IDataGridColumnBuilder
    {
        ...
        private DataTemplate GetDataTemplate( Uri resourceUri )
        {
            ...
            template = this.templateFactory.FromResource( resourceUri );
            this.templates.TryAdd( resourceUri, template );

            return template;
        }

        private Style GetStyle( Uri resourceUri )
        {
            Contract.Requires( resourceUri != null );
            ...
            style = this.styleFactory.FromResource( resourceUri );
            this.styles.TryAdd( resourceUri, style );

            return style;
        }

        private ResourceDictionary GetResourceDictionary( Uri resourceUri )
        {
            Contract.Requires( resourceUri != null );
            ...
            resources = this.resourceDictionaryFactory.FromResource( resourceUri );
            this.resourceDictionaries.TryAdd( resourceUri, resources );

            return resources;
        }

        private DataTemplate GetDataTemplateFromResource( Uri resourceUri, string resourceKey )
        {
            Contract.Requires( resourceUri != null );
            ...
            var resources = this.GetResourceDictionary( resourceUri );
        ...
        private Style GetStyleFromResource( Uri resourceUri, string resourceKey )
        {
            Contract.Requires( resourceUri != null );
            Contract.Requires( !string.IsNullOrEmpty( resourceKey ) );
            Contract.Ensures( Contract.Result<Style>() != null );
        ...
        private void ApplyTemplates( Type type, DataGridTemplateColumn column, DataGridTemplateColumnAttribute attribute )
        {
            ...
            if ( string.IsNullOrEmpty( attribute.ResourceDictionary ) )
            {
                ...
                if ( !string.IsNullOrEmpty( attribute.CellTemplateName ) )
                    column.CellTemplate = this.GetDataTemplate( type.CreatePackUri( attribute.CellTemplateName ) );

                // resolve template from specified assembly
                if ( !column.IsReadOnly && !string.IsNullOrEmpty( attribute.CellEditingTemplateName ) )
                    column.CellEditingTemplate = this.GetDataTemplate( type.CreatePackUri( attribute.CellEditingTemplateName ) );
            }
            else
            {
                ...
                if ( !string.IsNullOrEmpty( attribute.CellTemplateName ) )
                    column.CellTemplate = this.GetDataTemplateFromResource( resourceUri, attribute.CellTemplateName );

                // resolve template from resource dictionary
                if ( !column.IsReadOnly && !string.IsNullOrEmpty( attribute.CellEditingTemplateName ) )
                    column.CellEditingTemplate = this.GetDataTemplateFromResource( resourceUri, attribute.CellEditingTemplateName );
            }
        }

        private void ApplyStyles( Type type, DataGridColumn column, DataGridElementStyleAttribute attribute )
        {
            Contract.Requires( type != null );
            Contract.Requires( column != null );
            Contract.Requires( attribute != null );
            ...
            if ( string.IsNullOrEmpty( attribute.ResourceDictionary ) )
            {
                ...
                if ( !string.IsNullOrEmpty( attribute.ElementStyleName ) )
                    boundColumn.ElementStyle = this.GetStyle( type.CreatePackUri( attribute.ElementStyleName ) );

                // resolve template from specified assembly
                if ( !boundColumn.IsReadOnly && !string.IsNullOrEmpty( attribute.EditingElementStyleName ) )
                    boundColumn.EditingElementStyle = this.GetStyle( type.CreatePackUri( attribute.EditingElementStyleName ) );
            }
            else
            {
                // resolve resource dictionary from specified assembly
                ...
                // resolve template from resource dictionary
                if ( !string.IsNullOrEmpty( attribute.ElementStyleName ) )
                    boundColumn.ElementStyle = this.GetStyleFromResource( resourceUri, attribute.ElementStyleName );

                // resolve template from resource dictionary
                if ( !column.IsReadOnly && !string.IsNullOrEmpty( attribute.EditingElementStyleName ) )
                    boundColumn.EditingElementStyle = this.GetStyleFromResource( resourceUri, attribute.EditingElementStyleName );
            }
        }
        ...
        private IEnumerable<DataGridColumn> GetColumns( Type type, string adapterPath, ColumnBuildOrders buildOrders )
        {
            Contract.Requires( type != null );
            Contract.Requires( adapterPath != null );
            Contract.Ensures( Contract.Result<IEnumerable<DataGridColumn>>() != null );
            ...
            foreach ( var property in GetPropertiesInOrder( type, buildOrders ) )
            {
                ...
                if ( template == null )
                {
                    ...
                    if ( style != null )
                        this.ApplyStyles( type, column, style );
                ...
                else
                {
                    ...
                    this.ApplyTemplates( type, column, template );
        ...
        /// appending the columns resolved by the builder.
        /// </summary>
        /// <param name="columns">The <see cref="ICollection{T}"/> to append the columns to.</param>
        [SuppressMessage( "Microsoft.Design", "CA1062:Validate arguments of public methods", MessageId = "0", Justification = "Validated by a code contract" )]
        public virtual void Buildup( ICollection<DataGridColumn> columns )
        {
            this.Buildup( columns, ColumnBuildOrders.Default );
        }
        ...
        [SuppressMessage( "Microsoft.Design", "CA1062:Validate arguments of public methods", MessageId = "0", Justification = "Validated by a code contract" )]
        public virtual void Buildup( ICollection<DataGridColumn> columns, ColumnBuildOrders buildOrders )
        {
            columns.Clear();
            this.AppendTo( columns, buildOrders );
        }
        ...
        [SuppressMessage( "Microsoft.Design", "CA1062:Validate arguments of public methods", MessageId = "0", Justification = "Validated by a code contract" )]
        public virtual void AppendTo( ICollection<DataGridColumn> columns )
        {
            this.AppendTo( columns, ColumnBuildOrders.Default );
        }
        ...
        /// <param name="buildOrders">One or more of the <see cref="ColumnBuildOrders"/> values.</param>
        [SuppressMessage( "Microsoft.Design", "CA1062:Validate arguments of public methods", MessageId = "0", Justification = "Validated by a code contract" )]
        public virtual void AppendTo( ICollection<DataGridColumn> columns, ColumnBuildOrders buildOrders )
        {
            columns.AddRange( this.GetColumns( this.TargetType, this.AdapterBindingPath, buildOrders ) );
        }
    }
}
