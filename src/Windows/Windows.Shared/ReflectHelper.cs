namespace More
{
    ...
    {
        private static bool ParseProperty( string propertyName )
        {
            Contract.Requires( !string.IsNullOrEmpty( propertyName ) );
            return Regex.IsMatch( propertyName, @"^[A-Za-z][A-Za-z0-9_]*$", RegexOptions.Singleline );
        }

        private static bool ParseOrdinalIndexer( string path, out string propertyName, out int index )
        {
            Contract.Requires( !string.IsNullOrEmpty( path ) );
            propertyName = null;
            index = -1;

            var match = Regex.Match( path, @"^([A-Za-z][A-Za-z0-9_]*)?\s*\[\s*(\d+)\s*\]$", RegexOptions.Singleline );
        ...
        private static bool ParseStringIndexer( string path, out string propertyName, out string key )
        {
            Contract.Requires( !string.IsNullOrEmpty( path ) );
            propertyName = null;
            key = null;

            var match = Regex.Match( path, @"^([A-Za-z][A-Za-z0-9_]*)?\s*\[([^\]]+)\]$", RegexOptions.Singleline );
        ...
        private static object InvokeProperty( object target, string propertyName, object[] args )
        {
        ...
        private static object InvokeProperty( object target, string propertyPath )
        {
            Contract.Requires( target != null );
            ...
            {
                // invoke normal property
                return InvokeProperty( target, propertyPath, null );
            }
            else if ( ParseOrdinalIndexer( propertyPath, out propertyName, out index ) )
            {
                // get property with indexer (this could be on target itself)
            ...
            else if ( ParseStringIndexer( propertyPath, out propertyName, out key ) )
            {
                // get property with indexer (this could be on target itself)
                if ( !string.IsNullOrEmpty( propertyName ) )
                {
        ...
        private static PropertyDescriptor InvokePropertySetter( object target, string propertyName, object value, object[] args )
        {
            Contract.Requires( target != null );
            Contract.Requires( !string.IsNullOrEmpty( propertyName ) );
            ...
            try
            {
                // convert the value to the property type as necessary and set the value
        ...
        private static PropertyDescriptor InvokePropertySetter( object target, string propertyPath, object value )
        {
            Contract.Requires( target != null );
            Contract.Requires( !string.IsNullOrEmpty( propertyPath ) );
        ...
        private static MethodInfo GetMatchingMethod( Type type, string methodName, IList<MethodInfo> methods, IEnumerable<MethodParameter> parameters )
        {
            ...
            // short-circuit if there's only one method
            if ( methods.Count == 1 )
                return methods[0];

            // build method parameter types using defined type or infer from parameter values when possible
            ...
            // let reflection do it's magic by matching the method with the appropriate arguments
        ...
        internal static object Invoke( this MethodInfo method, object target, IEnumerable<MethodParameter> parameters )
        {
            ...
            // convert the argument to the parameter type as necessary
